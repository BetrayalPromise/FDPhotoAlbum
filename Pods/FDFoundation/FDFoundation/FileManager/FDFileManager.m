...
+(BOOL)createDirectoriesForFileAtPath:(NSString *)path error:(NSError **)error
{
    ...
    {
        //???: 下面 NSException 在Swift中调用 无法Handle其异常，所以注释下面的代码，用返回值来标识是否成功。
...
{
    NSDate *date = (NSDate *)[self attributeOfItemAtPath:path forKey:NSFileCreationDate error:error];
    NSTimeZone *zone = [NSTimeZone systemTimeZone];
    NSTimeInterval interval = (NSTimeInterval)zone.secondsFromGMT;
...
{
    NSDate *date = (NSDate *)[self attributeOfItemAtPath:path forKey:NSFileModificationDate error:error];
    NSTimeZone *zone = [NSTimeZone systemTimeZone];
    NSTimeInterval interval = (NSTimeInterval)zone.secondsFromGMT;
    return [date dateByAddingTimeInterval:interval];
}
...
{
    return [[FDFileManager pathForApplicationSupportDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[FDFileManager pathForCachesDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[FDFileManager pathForDocumentsDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[FDFileManager pathForLibraryDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[FDFileManager pathForMainBundleDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[FDFileManager pathForTemporaryDirectory] stringByAppendingPathComponent:path];
}
