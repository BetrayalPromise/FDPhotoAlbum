#import "FDZipArchive.h"
#include "fd_unzip.h"
#include "fd_zip.h"
#include "fd_minishared.h"

#include <sys/stat.h>

NSString *const FDZipArchiveErrorDomain = @"FDZipArchiveErrorDomain";

#define CHUNK 16384

int _zipOpenEntry(zipFile entry, NSString *name, const zip_fileinfo *zipfi, int level, NSString *password, BOOL aes);
BOOL _fileIsSymbolicLink(const fd_unz_file_info *fileInfo);
...
@interface NSData(FDZipArchive)
- (NSString *)_base64RFC4648 API_AVAILABLE(macos(10.9), ios(7.0), watchos(2.0), tvos(9.0));
- (NSString *)_hexString;
@end

@interface NSString (FDZipArchive)
- (NSString *)_sanitizedPath;
@end

@interface FDZipArchive ()
- (instancetype)init NS_DESIGNATED_INITIALIZER;
@end

@implementation FDZipArchive
{
    /// path for zip file
    NSString *_path;
    zipFile _zip;
...
+ (BOOL)isPasswordValidForArchiveAtPath:(NSString *)path password:(NSString *)pw error:(NSError **)error {
    ...
    if (zip == NULL) {
        if (error) {
            *error = [NSError errorWithDomain:FDZipArchiveErrorDomain
                                         code:FDZipArchiveErrorCodeFailedOpenZipFile
        ...
        do {
            ...
            if (ret != UNZ_OK) {
                if (ret != UNZ_BADPASSWORD) {
                    if (error) {
                        *error = [NSError errorWithDomain:FDZipArchiveErrorDomain
                                                     code:FDZipArchiveErrorCodeFailedOpenFileInZip
            ...
            if (ret != UNZ_OK) {
                if (error) {
                    *error = [NSError errorWithDomain:FDZipArchiveErrorDomain
                                                 code:FDZipArchiveErrorCodeFileInfoNotLoadable
                ...
                if (readBytes < 0) {
                    ...
                    if (readBytes != Z_DATA_ERROR) {
                        if (error) {
                            *error = [NSError errorWithDomain:FDZipArchiveErrorDomain
                                                         code:FDZipArchiveErrorCodeFileContentNotReadable
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(nullable id<FDZipArchiveDelegate>)delegate
{
          ...
          toDestination:(NSString *)destination
              overwrite:(BOOL)overwrite
               password:(nullable NSString *)password
                  error:(NSError **)error
               delegate:(nullable id<FDZipArchiveDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path
        ...
        progressHandler:(void (^)(NSString *entry, fd_unz_file_info zipInfo, long entryNumber, long total))progressHandler
...
+ (BOOL)unzipFileAtPath:(NSString *)path
          toDestination:(NSString *)destination
        progressHandler:(void (^_Nullable)(NSString *entry, fd_unz_file_info zipInfo, long entryNumber, long total))progressHandler
      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    ...
    if (path.length == 0 || destination.length == 0)
    {
        ...
        NSError *err = [NSError errorWithDomain:FDZipArchiveErrorDomain code:FDZipArchiveErrorCodeInvalidArguments userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (zip == NULL)
    {
        ...
        NSError *err = [NSError errorWithDomain:FDZipArchiveErrorDomain code:FDZipArchiveErrorCodeFailedOpenZipFile userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (ret != UNZ_OK && ret != UNZ_END_OF_LIST_OF_FILE)
    {
        ...
        NSError *err = [NSError errorWithDomain:FDZipArchiveErrorDomain code:FDZipArchiveErrorCodeFailedOpenFileInZip userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    do {
        ...
        @autoreleasepool {
            ...
            if (ret != UNZ_OK) {
                unzippingError = [NSError errorWithDomain:@"FDZipArchiveErrorDomain" code:FDZipArchiveErrorCodeFailedOpenFileInZip userInfo:@{NSLocalizedDescriptionKey: @"failed to open file in zip file"}];
                success = NO;
                break;
            }
            ...
            fd_unz_file_info fileInfo;
            memset(&fileInfo, 0, sizeof(fd_unz_file_info));
            
            ret = unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            if (ret != UNZ_OK) {
                unzippingError = [NSError errorWithDomain:@"FDZipArchiveErrorDomain" code:FDZipArchiveErrorCodeFileInfoNotLoadable userInfo:@{NSLocalizedDescriptionKey: @"failed to retrieve info for file"}];
                success = NO;
                unzCloseCurrentFile(zip);
                break;
            }
            ...
            NSString * strPath = [FDZipArchive _filenameStringWithCString:filename
            ...
            if (err != nil) {
                ...
                NSLog(@"[FDZipArchive] Error: %@", err.localizedDescription);
            }
                        ...
                        if (readBytes > 0) {
                            if (0 == fwrite(buffer, readBytes, 1, fp)) {
                                if (ferror(fp)) {
                                    ...
                                    NSLog(@"[FDZipArchive] %@", message);
                                    success = NO;
                                    unzippingError = [NSError errorWithDomain:@"FDZipArchiveErrorDomain" code:FDZipArchiveErrorCodeFailedToWriteFile userInfo:@{NSLocalizedDescriptionKey: message}];
                                    break;
                                }
                            }
                        } else {
                        ...
                        if (nestedZipLevel
                            ...
                            if (fileInfo.dos_date != 0) {
                                ...
                                if (attr) {
                                    if (![fileManager setAttributes:attr ofItemAtPath:fullPath error:nil]) {
                                        ...
                                        NSLog(@"[FDZipArchive] Failed to set attributes - whilst setting modification date");
                                    }
                                }
                            }
                            ...
                            if (permissions != 0) {
                                ...
                                if (![fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil]) {
                                    ...
                                    NSLog(@"[FDZipArchive] Failed to set attributes - whilst setting permissions");
                                }
                            }
                        }
                    }
                ...
                if (overwrite)
                {
                    if ([fileManager fileExistsAtPath:fullPath])
                    {
                        ...
                        if (!removeSuccess)
                        {
                            ...
                            NSLog(@"[FDZipArchive] %@", message);
                            success = NO;
                            unzippingError = [NSError errorWithDomain:FDZipArchiveErrorDomain code:error.code userInfo:@{NSLocalizedDescriptionKey: message}];
                        }
                    }
                }
    ...
    if (success && preserveAttributes) {
        ...
        for (NSDictionary * d in directoriesModificationDates) {
            if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: [d objectForKey:@"modDate"]} ofItemAtPath:[d objectForKey:@"path"] error:&err]) {
                NSLog(@"[FDZipArchive] Set attributes failed for directory: %@.", [d objectForKey:@"path"]);
            }
            if (err) {
                NSLog(@"[FDZipArchive] Error setting directory file modification date attribute: %@", err.localizedDescription);
            }
        }
    }
    ...
    if (crc_ret == UNZ_CRCERROR)
    {
        ...
        retErr = [NSError errorWithDomain:FDZipArchiveErrorDomain code:FDZipArchiveErrorCodeFileInfoNotLoadable userInfo:userInfo];
    }
...
{
    return [FDZipArchive createZipFileAtPath:path withFilesAtPaths:paths withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray<NSString *> *)paths diskSize:(int)diskSize{
    FDZipArchive *zipArchive = [[FDZipArchive alloc] initWithPath:path];
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath {
    return [FDZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory {
    return [FDZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory withPassword:nil];
}
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath withPassword:(nullable NSString *)password {
    return [FDZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:NO withPassword:password];
}


+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory withPassword:(nullable NSString *)password {
    return [FDZipArchive createZipFileAtPath:path
...
- (BOOL)writeFolderAtPath:(NSString *)path withFolderName:(NSString *)folderName withPassword:(nullable NSString *)password
{
    ...
    [FDZipArchive zipInfo:&zipInfo setAttributesOfItemAtPath:path];
...
- (BOOL)writeData:(NSData *)data filename:(nullable NSString *)filename compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    zip_fileinfo zipInfo = {};
    [FDZipArchive zipInfo:&zipInfo setDate:[NSDate date]];
...
- (BOOL)close
{
    NSAssert((_zip != NULL), @"[FDZipArchive] Attempting to close an archive which was never opened");
...
{
    NSCalendar *currentCalendar = FDZipArchive._gregorian;
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime
{
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[FDZipArchive] MSDOS date masks don't add up");
...
@implementation NSData (FDZipArchive)
...
@implementation NSString (FDZipArchive)
